...
FACES = [("B","C",PENT_H,"REMOVABLE PANEL (future bridge)"),
         ("C","D",DIAG_LEN,"DIAGONAL — FIXED GUARD / ROUND DOOR BELOW"),
         ("D","E",PENT_H,"SELF-CLOSING GATE")]
...
def developed_elev(sh):
    ...
    sh.circle((dc, HDOOR_CZ), HDOOR_RO_W/2, fill=None, stroke=HexColor("#8a7f66"), w=1.0)
    sh.circle((dc, HDOOR_CZ), HDOOR_OPEN_D/2, fill=HexColor("#3a5a3c"), stroke=INK, w=1.4)
    sh.circle((dc, HDOOR_CZ), HDOOR_SLAB_D/2, fill=None, stroke=HexColor("#94b08f"), w=0.8)
    for s_ in (-1, 1):
        yy = HDOOR_CZ + s_*HDOOR_SLAB_D*0.26
        halfw = math.sqrt(max((HDOOR_SLAB_D/2)**2 - (yy-HDOOR_CZ)**2, 0)) - 1.0
        sh.poly([(dc-halfw, yy-0.8), (dc+halfw*0.55, yy-0.8),
                 (dc+halfw*0.55, yy+0.8), (dc-halfw, yy+0.8)], fill=INK, stroke=None)
    sh.circle((dc+HDOOR_SLAB_D/2-3.2, HDOOR_CZ), 1.3, fill=HexColor("#4a3520"), stroke=INK, w=0.6)
    sh.dim((ro0, T_2X), (ro1, T_2X), -9, f'SQUARE R.O. {frac(HDOOR_RO_W)}')
    sh.dim((ro1+0.6, T_2X), (ro1+0.6, T_2X+HDOOR_RO_H), 8, frac(HDOOR_RO_H))
    sh.leader((dc, HDOOR_CZ+HDOOR_OPEN_D/2), (-30, 30),
              f'TRUE CIRCLE {frac(HDOOR_OPEN_D)} opening / {frac(HDOOR_SLAB_D)} slab', 6.0, anchor="r")
    sh.leader((dc-HDOOR_OPEN_D/2, HDOOR_SILL), (-34, -26),
              f'step-over sill {frac(HDOOR_SILL)}', 6.0, anchor="r")

    # porthole on face 2
    px0 = _face_x0(0)
    sh.circle((px0+PENT_H*0.5, PORTHOLE_Z), PORTHOLE_D/2, fill=HexColor("#cfe0e2"), stroke=INK, w=1.0)
    sh.circle((px0+PENT_H*0.5, PORTHOLE_Z), PORTHOLE_D/2+0.9, fill=None, stroke=INK, w=1.4)
    sh.leader((px0+PENT_H*0.5, PORTHOLE_Z), (-40, -36),
...
def kneewall_and_door(sh):
    ...
    # ---- arch template ----
    tw = HDOOR_SLAB_D + 10
    sh.view(62+(sh.W-410)*0.54, 116, (sh.W-410)*0.44, sh.H-236, -5, -8, tw, HDOOR_SLAB_D+14)
    for gx in range(0, int(tw)+1, 2):
        sh.line((gx, 0), (gx, HDOOR_SLAB_D+2), GRID, 0.4)
    for gy in range(0, int(HDOOR_SLAB_D)+3, 2):
        sh.line((0, gy), (tw, gy), GRID, 0.4)
    for gx in range(0, int(tw)+1, 6):
        sh.line((gx, 0), (gx, HDOOR_SLAB_D+2), HexColor("#c9c3b4"), 0.7)
        sh.txt((gx, -4), f'{gx}"', 5.2, THIN, "c")
    for gy in range(0, int(HDOOR_SLAB_D)+3, 6):
        sh.line((0, gy), (tw, gy), HexColor("#c9c3b4"), 0.7)
        sh.txt((-1.5, gy-1), f'{gy}"', 5.2, THIN, "r")
    dc = tw/2; sdz = HDOOR_SLAB_D - (HDOOR_SLAB_D/2)
    pts = [(dc-HDOOR_SLAB_D/2, 0), (dc+HDOOR_SLAB_D/2, 0), (dc+HDOOR_SLAB_D/2, sdz)]
    pts += [(dc+(HDOOR_SLAB_D/2)*math.cos(math.radians(t*3.75)), sdz+(HDOOR_SLAB_D/2)*math.sin(math.radians(t*3.75)))
            for t in range(49)]
    pts += [(dc-HDOOR_SLAB_D/2, sdz)]
    sh.poly(pts, fill=HexColor("#e7eadf"), stroke=INK, w=1.8)
    # plank lines
    for pk in range(1, 5):
        px = dc - HDOOR_SLAB_D/2 + HDOOR_SLAB_D*pk/5
        sh.line((px, 0.4), (px, sdz+math.sqrt(max((HDOOR_SLAB_D/2)**2-(px-dc)**2, 0))-0.4), HexColor("#9aa48c"), 0.8)
    sh.circle((dc, sdz), 0.6, fill=RED, stroke=None)
    sh.line((dc, sdz), (dc+(HDOOR_SLAB_D/2), sdz), RED, 0.7, (3,2))
    sh.txt((dc+(HDOOR_SLAB_D/2)/2, sdz+1.4), f'R {frac((HDOOR_SLAB_D/2))}', 6.4, RED, "c", True)
    sh.dim((dc-HDOOR_SLAB_D/2, -1), (dc+HDOOR_SLAB_D/2, -1), -12, frac(HDOOR_SLAB_D))
    sh.dim((dc+HDOOR_SLAB_D/2+1, 0), (dc+HDOOR_SLAB_D/2+1, HDOOR_SLAB_D), 14, frac(HDOOR_SLAB_D))
    sh.dim((dc+HDOOR_SLAB_D/2+1, 0), (dc+HDOOR_SLAB_D/2+1, sdz), 40, f'spring line {frac(sdz)}')
    for s in (-1, 1):
        sh.poly([(dc-HDOOR_SLAB_D/2+0.8, sdz/2+s*sdz*0.30), (dc+HDOOR_SLAB_D/2*0.7, sdz/2+s*sdz*0.30),
                 (dc+HDOOR_SLAB_D/2*0.7, sdz/2+s*sdz*0.30+1.6), (dc-HDOOR_SLAB_D/2+0.8, sdz/2+s*sdz*0.30+1.6)],
                fill=INK, stroke=None)
    sh.circle((dc+HDOOR_SLAB_D/2-3.5, sdz*0.60), 1.4, fill=HexColor("#4a3520"), stroke=INK, w=0.7)
    sh.txt((dc, HDOOR_SLAB_D+10), "HOBBIT DOOR — FULL TEMPLATE", 9, INK, "c", True)
    sh.txt((dc, HDOOR_SLAB_D+4), '2" grid — scale off the grid, not the paper', 6.2, THIN, "c")
